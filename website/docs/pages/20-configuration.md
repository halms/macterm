...
Macterm reads your `~/.config/ghostty/config.ghostty` on launch, falling back to the legacy `~/.config/ghostty/config` when that is the only one present. Themes, fonts, palettes, keybinds, and everything else Ghostty supports work the same here. If your config lives elsewhere, set the path in **Settings → General → Ghostty Config**.
